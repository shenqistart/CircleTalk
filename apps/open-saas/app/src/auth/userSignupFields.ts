...
function getVerifiedGoogleEmail(data: unknown) {
  const googleData = googleDataSchema.parse(data);
  if (!googleData.profile.email_verified) {
    throw new Error("Google account email must be verified to sign up.");
  }
...
export const getGoogleUserFields = defineUserSignupFields({
  email: (data) => {
    return getVerifiedGoogleEmail(data);
  },
  username: (data) => {
    return getVerifiedGoogleEmail(data);
  },
  isAdmin: (data) => {
    return adminEmails.includes(getVerifiedGoogleEmail(data));
  },
});
