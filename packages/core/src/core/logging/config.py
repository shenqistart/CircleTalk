...
class ColoredFormatter(MillisecondFormatter):
    """为不同日志级别提供颜色的格式化器（Spring Boot 风格）"""
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        # Spring Boot 风格：WARNING -> WARN, CRITICAL -> FATAL，右对齐 5 字符
        display_name = self.LEVEL_MAP.get(original_levelname, original_levelname)
        padded_name = display_name.rjust(5)
        color = self.COLORS.get(record.levelno, AnsiColor.RESET)
        record.levelname = AnsiColor.colorize(padded_name, color)
...
class SmartExceptionFormatter(ColoredFormatter):
    ...
    def formatException(self, ei: tuple[type[BaseException] | None, BaseException | None, TracebackType | None]) -> str:
        ...
        for line in tb_lines[1:-1]:
            if any(path in line for path in LogConfig.EXCEPTION_FILTER_PATHS):
...
class LogConfig:
    ...
    LOG_FORMAT = (
        "%(asctime)s %(levelname)s %(process)d --- [%(threadName)-15s] [%(request_id)s] %(name)-45s : %(message)s"
    )
    ...
    BACKUP_COUNT = int(os.getenv("LOG_BACKUP_COUNT", "100"))
    ...
    EXCEPTION_FILTER_PATHS: ClassVar[list[str]] = [
    ...
    SLOW_THRESHOLDS: ClassVar[dict[str, int]] = {
        "http": 3000,
        "db": 500,
        "llm": 30000,
        "cube": 5000,
