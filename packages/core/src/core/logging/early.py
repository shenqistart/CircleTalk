...
from core.logging.config import ColoredFormatter, LogConfig
...
def _configure_early_logging() -> None:
    ...
    功能：
    1. 添加 StreamHandler 输出到 stdout（与主日志系统一致）
    2. 使用 Spring Boot 风格格式（与主日志系统一致）
    3. 添加 RequestContextFilter 注入 request_id
    4. 设置 INFO 级别
    5. 幂等性保护（只运行一次）
    """
    if get_logging_phase() != LoggingPhase.UNINITIALIZED:
        return
    root_logger = logging.getLogger()
    if not root_logger.handlers:
        handler = logging.StreamHandler(sys.stdout)
        handler.setFormatter(ColoredFormatter(LogConfig.LOG_FORMAT))
        handler.addFilter(RequestContextFilter())
        root_logger.addHandler(handler)
        root_logger.setLevel(logging.INFO)
    set_logging_phase(LoggingPhase.EARLY)
